...
export function applyUserDataSchema(db) {
...
export function ensureOwnerUserRow(db, userId, email, isAdmin) {
    const existing = db.prepare('SELECT id FROM users WHERE id = ?').get(userId);
    if (existing) {
        return;
    }
    db.prepare('INSERT INTO users (id, email, password_hash, password_salt, is_admin) VALUES (?, ?, ?, ?, ?)').run(userId, email, '', '', isAdmin ? 1 : 0);
}
